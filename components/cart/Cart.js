        ...
        {items.length > 0 ? (
          <ul>
            {items.map((cartItem, index) => (
              <li key={index} className="mb-4 p-2 flex justify-between items-center border-b">
                <div className="flex items-center">
                  <input 
                    type="number" 
                    value={cartItem.quantity} 
                    onChange={(e) => updateQuantity(cartItem.item, parseInt(e.target.value))}
                    className="form-input mx-2 text-center w-16"
                    min="1"
                  />
                  <span className="mr-2 font-bold">{cartItem.item.name}</span>
                  <span className="text-sm text-gray-600">Cena: {cartItem.item.price * cartItem.quantity} zł</span>
                </div>
                <button
            ...
            <div className="text-lg font-bold mt-4">Łączna suma: {getTotalSum()} zł</div>
          </ul>
        ) : (
